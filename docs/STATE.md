...
- [/] inventory-stream-collision-avoidance: Inventory-stream reframing and collision avoidance (plans/COLLISION_AVOIDANCE_TRACK.md) — Step 1 (gate) PASSED 2026-09-25: session openings are near-universal in the raw HTR (date region 100% of 1,690 raw sessions, first region 99.8%, attendance 71.7%) and localizable on the paragraph_axis stream for 86.0% of 1,316 flat sessions across three complementary channels (fingerprint-verified 71.4%, formulaic openings 38.0%, in-axis president/present 18.0%). Decisive result: content-verified landmarks are strictly order-preserving in all five annual inventories (0 non-monotone steps) while 80-99% of session labels carry a non-zero offset in only 5-11 piecewise runs -- the label is wrong nearly everywhere, the order is intact everywhere. Locality cost is 2.13 dates/segment and median segment k_e 1.33x the day median; headroom is +1,329 on the 11,644 ceiling, but only +342 once the 535 no-HTR dates are barred from borrowing a neighbour's paragraphs, i.e. below the 718 Global-stretch gap. Step 2 (length-proportional prior) already closed NEGATIVE. dataset landmark_density_eval.
