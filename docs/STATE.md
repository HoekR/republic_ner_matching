...
- [/] s6-anchor-chain-alignment: S6 -- multi-channel anchor chaining at character coordinates — 2026-09-23: session-uniqueness-policy fix (resolve_row dedup) built, measured, and confirmed collision-freeness alone does not move this track's metrics (separated share and weak_separation day count both flat after full chain re-run) -- rules out mapping as the binding constraint, strengthens the case that this is a placement-quality problem. User greenlit the segmentation DP as the next multi-session investment; spans criterion downgraded from readiness gate to non-blocking the same session.
