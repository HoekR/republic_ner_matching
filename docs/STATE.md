...
- **s6-anchor-chain-alignment — Corpus-wide (not window-sampled) cross-tab: resolved_session_id collision dates (153, session claimed by 2+ enriched dates) vs Tier O weak_separation classification, over all 1,138 resolved_auto days. Collision dates are weak_separation 80.4% of the time vs. 32.7% for non-collision dates (2.5x). 123/445 (27.6%) of ALL weak_separation days corpus-wide are collision-involved -- the population-level answer to how much of the span-blocking weak_separation population this one mapping bug explains. Higher is worse (more of the blocking population explained by a fixable mapping bug, which is actually a favorable reading for tractability).:** 0.2764
