...
| **S6c** — count-constrained segmentation DP (RESCOPED 2026-09-21, `docs/DECISIONS.md`) | **DONE 2026-09-21** — [scripts/s6c_gap_segmentation.py](../../scripts/s6c_gap_segmentation.py) (`segment_gap`/`segment_day`, 11/11 tests), swapped in for `interpolate_positions` in both `s4_paragraph_axis_baseline.py` and `s4_corpus_paragraph_predictions.py`; `interpolate_positions` itself is untouched (`s6_oracle_anchor_diagnostic.py` still needs the unmodified model). Never abstains: a non-decreasing anchor backbone bounded by session start/end sentinels, with repeats allowed when a gap is narrower than its resolution count, and group-C phrase hits as soft in-gap evidence at gold-day scale. Gold-day result: predicted-day coverage 9/21 → 19/21 eligible (7/19 → 19/19 scoreable), lumped F1 flat (0.828 → 0.826). Corpus-wide result: `insufficient_entity_anchors` 777 → **0**, 100% of days with an axis now predict; quality caveat — 79/1059 (7.5%) predicted days collapse 7+ resolutions onto one paragraph (low-localization coverage, not real segmentation). Corpus-scale group-C phrase-hit wiring was **tried and measured negative 2026-09-21** (`docs/DECISIONS.md`): paragraph tol0 F1 0.644 → 0.622 on the gold-day sample scored through the corpus predictor's own codepath, mean Pk/WindowDiff also worse; not wired in. **Group-B evidence (`entity_surface_matches_1626_1630`) was also tried and measured negative, 2026-09-23** (`docs/DECISIONS.md`, [scripts/s6_group_b_position_scores_eval.py](../../scripts/s6_group_b_position_scores_eval.py)): same 19/21 gold-day test, paragraph tol0 F1 0.644 → 0.555, worse than Group-C's own regression. Both evidence sources fail the same way through the same `position_scores` bonus-vs-even-split mechanism in `segment_gap` — entity/phrase presence signals content, not where a resolution *opens* — which points at the mechanism, not the evidence choice. `position_scores` tuning is likely exhausted for closing `weak_separation`/severe-collapse without a qualitatively different in-gap signal; not a candidate to keep iterating on without new evidence. (Metric granularity note: both results are scored in **paragraph** units, not character-offset tolerance — `docs/DECISIONS.md` 2026-09-21 "S6c target metric rescoped" already ruled out char-tolerance as this track's target.) Day-level **grounding** of Group-B (restricting the bonus to matches a date's own enriched resolutions actually claim) partially recovers but does not close the regression: tol0 0.555 → 0.588, still below baseline (`docs/DECISIONS.md` 2026-09-23 "Day-level grounding of Group-B partially recovers"). Per-paragraph **`.find()`-relocation** of Group-B (fixing the flat-id-vs-paragraph-id granularity mismatch directly, via `scripts/s6_group_b_relocated_position_scores_eval.py`) was tried and also measured negative, 2026-09-23: tol0 0.543 (fuzzy fallback) / 0.560 (exact-only control) — both still below the blanket version's own 0.555, ruling out granularity as the dominant cause after all. `position_scores` tuning is now exhausted across four tested variants (blanket, grounded, relocated, exact-relocated); the two remaining untried levers are (b) per-entity role/type weighting in `segment_gap`'s cost function and (c) the full two-sided entity-overlap consolidation table — neither started without an explicit go/no-go. **User picked (c), 2026-09-23.** Ran its cheap gold-day-harness precursor before committing to the full 4-phase migration: [scripts/s6_group_b_candidate_grounded_position_scores_eval.py](../../scripts/s6_group_b_candidate_grounded_position_scores_eval.py) (5 tests) grounds Group-B's bonus to only the specific enriched resolution(s) that `segment_day`'s count-constrained DP already knows occupy a given gap (non-circular — the DP knows the gap's candidate set from its `count` alone, before choosing *where* within it), the tightest two-sided grounding possible without building the table. Result: tol0 F1 **0.588 — ties the already-tested day-level grounding exactly** (`candidate_id_match_rate=1.0`, 87/87, so not a data-matching artifact), tol1 0.779 (day-grounded 0.784), tol2 0.829 (day-grounded 0.824). **Do not build the full two-sided consolidation table** — it would deliver exactly this same per-candidate-grounding property via far more machinery, and this cheap test found no headroom there. `position_scores` tuning is now exhausted across five variants; only (b) remains untried. |
