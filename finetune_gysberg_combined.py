#!/usr/bin/env python3
"""
Fine-tune GysBERT on combined LOC + PER annotations (places + delegate names).

Uses ground-truth location and person spans from LOC-annotations and PER-annotations
for accurate multi-class NER training (PLACE + NAME).
...
# Configuration
DATA_DIR = Path("data")
LOC_PAIRS_FILE = DATA_DIR / "training_pairs_loc_1626_1630_dedup.parquet"
PER_PAIRS_FILE = DATA_DIR / "training_pairs_per_1626_1630_dedup.parquet"
OUTPUT_DIR = Path("models/gysberg_delegate_ner_combined")
...
def load_training_pairs():
    """Load both LOC and PER annotated training pairs."""
    print("Loading training pairs...")
    
    # Load LOC (places)
    loc_df = pd.read_parquet(LOC_PAIRS_FILE)
    print(f"  LOC pairs: {len(loc_df)}")
    
    # Load PER (delegates)
    per_df = pd.read_parquet(PER_PAIRS_FILE)
    print(f"  PER pairs: {len(per_df)}")
    
    # Combine
    df = pd.concat([loc_df, per_df], ignore_index=True)
...
def format_sentence_with_labels(group_df):
    """
    Convert annotated training pairs into labeled sentences.
    
    For each resolution, create a sentence and label all entities (PLACE + NAME).
    Uses exact word-boundary matching.
    
    Returns list of Sentence objects with PLACE/NAME labels.
    """
    sentences = []
            ...
            for token in sentence.tokens:
                ...
                for mention in mentions:
                    ...
                    if (token_text == htr_lower or token_text == canonical_lower):
                        token.add_label('ner', mention['label'])
                        is_entity = True
                        break
...
def main():
    ...
    print("=" * 70)
    print("GysBERT Fine-tuning on Combined LOC+PER (Places + Delegates)")
