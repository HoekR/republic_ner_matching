#!/usr/bin/env python3
"""
GPU-optimized fine-tuning of GysBERT on PLACE names (LOC annotations).
...
# Paths
DATA_DIR = Path("data")
MODEL_DIR = Path("models/gysberg_place_ner_gpu")
TRAINING_DATA = DATA_DIR / "training_pairs_loc_1626_1630_dedup.parquet"
...
def load_training_data():
    """Load LOC training pairs from parquet."""
...
def format_sentence_with_labels(group_df):
    ...
    Columns:
      - htr_span: the annotated entity text
    ...
    for para_idx, para_group in group_df.groupby('para_idx'):
        ...
        # Collect PLACE spans (character offsets in paragraph_text, not truncated_text)
        place_spans = []
        for _, row in para_group.iterrows():
            offset = row['offset']
            end = row['offset'] + len(row['htr_span'])
            place_spans.append((offset, end))
        ...
        for token in sentence.tokens:
            ...
            # Check if token overlaps with any PLACE span
            is_place = False
        ...
        # Only keep sentences with at least one PLACE token
        has_place = any(token.get_label('ner').value == 'PLACE' for token in sentence.tokens)
        if has_place and len(sentence.tokens) > 0:
...
def main():
    ...
    logger.info("Starting GysBERT fine-tuning on GPU")
