#!/usr/bin/env python3
"""
Memory-optimized fine-tuning of GysBERT on PER annotations (delegate names).

Strategy:
- Use PER-only data (13K pairs, delegate/person names)
- Character offset-based token labeling (htr_span → NAME spans)
- Batch size 8 (optimized for macOS MPS/CPU)
- Stream sentences without full caching
- Date-stratified split (70/15/15)
- 10 epochs for stability

Usage:
    uv run python finetune_gysberg_delegate_local.py
"""
...
BATCH_SIZE = 8  # Reduced from 16 to prevent OOM with corrected paragraph texts  # Increased for better convergence
MAX_EPOCHS = 7   # Reduced epochs
...
def format_sentence_with_labels(group_df):
    """
    Format sentences using character offsets to label tokens.
    ...
    for resolution_id, res_group in tqdm(group_df.groupby('resolution_id'),
        ...
        try:
            ...
            # Collect (offset, end) spans from annotations — these are character positions
            # PER annotations have htr_span, offset, end columns
            name_spans = []
            for _, row in res_group.iterrows():
                offset = row.get('offset')
                htr_span = row.get('htr_span', '')
                if offset is None or not htr_span:
                    continue
                end = offset + len(str(htr_span))
                if offset < MAX_CHARS:
                    name_spans.append((int(offset), min(int(end), MAX_CHARS)))

            if not name_spans:
                continue

            # Label each token by checking if its character position falls in a span
            # Flair tokens have .start_position and .end_position
            ...
            # Only keep sentences that have at least one NAME token
            if any(t.get_label('ner').value == 'NAME' for t in sentence.tokens):
...
def create_corpus(df):
    """Create corpus with date stratification or random split."""
...
def finetune_model(corpus):
    ...
    print("\nFine-tuning GysBERT (DELEGATE name recognition)...")
    ...
    trainer.train(
        ...
        patience=2,
        embeddings_storage_mode="none",  # keep embeddings on MPS, avoid CPU transfer conflict
...
def main():
    ...
    print("=" * 70)
    print("GysBERT Fine-tuning: Delegate Names (PER) — Memory Optimized")
